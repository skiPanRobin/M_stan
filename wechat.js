...
function _openWechat(payload){
    ...
    var parent微信 = className('android.widget.TextView').text('微信').findOne(10000).parent()
    text('通讯录').findOne(5000)
    if (parent微信 && parent微信.bounds().centerX() >100 && parent微信.bounds().centerY()>2000) {
        click(parent微信.bounds().centerX(), parent微信.bounds().centerY())
        sleep(800)
    } else {
        pressXY(129, 2210, 200, 800);  // 点击微信中的`微信`
    }
    
    autoSwipe(500, 459, 520, 2030, 500, 500)
    // 当appName为空时, 给出错误提示
    if (!payload.appName){
